...
@section('main')
    <section>
        <div class="caption-container ">
            <div>
                <div class="caption-container "
                     style="background: url({{asset($templateFile.'/images/img-cbd/title.jpg')}}) no-repeat;">
                        ...
                        <table class="table box table-bordered">
                            ...
                            @foreach($cart as $item)
                                ...
                                <tr class="row_cart">
                                    ...
                                    <td><input style="width: 70px;" type="number" data-id="{{ $item->id }}"
                                    ...
                                    <tr>
                                        ...
                                        <td class="form-group{{ $errors->has('ward') ? ' has-error' : '' }}">
                                            <label for="ward" class="control-label"><i
                                                        class="fa fa-list-ul"></i> {{ trans('cart.ward') }}</label>
                                            <input name="ward" type="text" placeholder="{{ trans('cart.ward') }}"
                                                   value="{{ (old('ward'))?old('ward'):$shippingAddress['ward']}}">
                                            @if($errors->has('ward'))
                                    ...
                                    <tr>
                                        <td class="form-group {{ $errors->has('district') ? ' has-error' : '' }}">
                                            <label for="district" class="control-label"><i
                                                        class="fa fa-list-ul"></i> {{ trans('cart.district') }}:</label>
                                            <input name="district" type="text"
                                                   placeholder="{{ trans('cart.district') }}"
                                                   value="{{ (old('district'))?old('district'):$shippingAddress['district']}}">
                                        ...
                                        <td class="form-group{{ $errors->has('province') ? ' has-error' : '' }}">
                                            <label for="province" class="control-label"><i
                                                        class="fa fa-list-ul"></i> {{ trans('cart.province') }}</label>
                                            <input name="province" type="text"
                                                   placeholder="{{ trans('cart.province') }}"
                                                   value="{{ (old('province'))?old('province'):$shippingAddress['province']}}">
                                            ...
                                            <div class="row">
                                                <div class="form-group col-md-6">
                                                    <label class="control-label" for="inputGroupSuccess3"><i
                                                                ...
                                                                aria-hidden="true"></i> {{ trans('cart.coupon') }}
                                                        <span style="display:inline; cursor: pointer; display: {{ ($hasCoupon)?'inline':'none' }}"
                                                              class="text-danger" id="removeCoupon">({{ trans('cart.remove_coupon') }} <i
                                                                    class="fa fa fa-times"></i>)</span>
                                                    </label>
                                                    <div id="coupon-group"
                                                         class="input-group {{ Session::has('error_discount')?'has-error':'' }}">
                                                        <input type="text"
                                                               {{ ($extensionDiscount['permission'])?'':'disabled' }} placeholder="Your coupon"
                                                               class="form-control" id="coupon-value"
                                                               aria-describedby="inputGroupSuccess3Status">
                                                        <span class="input-group-addon {{ ($extensionDiscount['permission'])?'':'disabled' }}"
                                                              {!! ($extensionDiscount['permission'])?'id="coupon-button"':'' !!} style="cursor: pointer;"
                                                              data-loading-text="<i class='fa fa-spinner fa-spin'></i> checking">{{ trans('cart.apply') }}</span>
                                                    </div>
                                                    <span class="status-coupon" style="display: none;"
                                                          class="glyphicon glyphicon-ok form-control-feedback"
                                                          aria-hidden="true"></span>
                                                    <div class="coupon-msg  {{ Session::has('error_discount')?'text-danger':'' }}"
                                                         style="text-align: left;padding-left: 10px;">{{ Session::has('error_discount')?Session::get('error_discount'):'' }}</div>
                                                </div>
                                            </div>
                                        @endif
                                        {{-- //End coupon --}}
                                            ...
                                            <div class="col-md-12">
                                                <div class="form-group {{ $errors->has('shippingMethod') ? ' has-error' : '' }}">
                                                    <h3 class="control-label"><i class="fa fa-truck"
                                                                                 aria-hidden="true"></i> {{ trans('cart.shipping_method') }}
                                                        :<br></h3>
                                                        ...
                                                        <div>
                                                            <label class="radio-inline">
                                                                <input type="radio" name="shippingMethod"
                                                                       value="{{ $shipping['code'] }}"
                                                                       {{ (old('shippingMethod') == $key)?'checked':'' }} style="position: relative;" {{ ($shipping['permission'])?'':'disabled' }}>
                                                                {{ $shipping['title'] }}
                                                        ...
                                                        <div>
                                                            <label class="radio-inline">
                                                                <input type="radio" name="paymentMethod"
                                                                       value="{{ $payment['code'] }}"
                                                                       {{ (old('paymentMethod') == $key)?'checked':'' }} style="position: relative;" {{ ($payment['permission'])?'':'disabled' }}>
                                    ...
                                    <div class="col-md-12 text-center">
                                        <div class="pull-right">
                                            <button class="btn btn-success" id="submit-order" type="button"
                                                    style="cursor: pointer;padding:10px 30px"><i class="fa fa-check"></i> {{ trans('cart.checkout') }}
                                            </button>
                                        </div>
                                    </div>
                                </div>
