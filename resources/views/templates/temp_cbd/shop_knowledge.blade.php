                ...
                <div class="row">
                    <div class="col-sm-9 col-md-9 blog-wrapper blog-style7">
                        <div class="bt-blog-posts">
                            <!-- Loop Start -->
                            @foreach ($knowledges as $knowledgesDetail)
                                <article class="post-item post-161 post type-post status-publish format-standard has-post-thumbnail hentry category-uncategorized">
                                    ...
                                    <div class="bt_thumbnail">
                                        <a href="{{ $knowledgesDetail->getUrl() }}">
                                            <img width="850" height="560"
                                                 src="{{ asset($knowledgesDetail->getThumb()) }}"
                                                 class="attachment-blog_post7 size-blog_post7 wp-post-image" alt=""
                                                 srcset="{{ asset($knowledgesDetail->getThumb()) }} 850w, {{ asset($knowledgesDetail->getThumb()) }} 300w, {{ asset($knowledgesDetail->getThumb()) }} 768w, {{ asset($knowledgesDetail->getThumb()) }} 600w"
                                    ...
                                    <div class="bt_content">
                                        <h3 class="blog-title">
                                            <a href="{{ $knowledgesDetail->getUrl() }}"
                                               rel="bookmark" title="Permanent Link to Enjoy your life with healthy body">{{ $knowledgesDetail->title }}</a>
                                        </h3>
                                        <div class="blog-subtitle">
                                            By <a href="{{ asset("/about.html")}}"
                                                  title="Posts by Abadmin" rel="author">Abadmin</a> <span
                                                    class="bt_blog_title_sep"> | </span>
                                            {{ $knowledgesDetail->created_at }} <span class="bt_blog_title_sep"> | </span>
                                            0 Comment <span class="bt_blog_title_sep"> | </span>
                                        </div>
                                        <div class="blog-content">
                                            {{ $knowledgesDetail->description }}
                                        </div>
                                    </div>
                                    <a class="bt_blog_widget-more"
                                       href="{{ $knowledgesDetail->getUrl() }}">Read  More</a>
                                </article>
                            @endforeach
                        </div>
