...
class ShopKnowledges extends Model
{
    public $table = 'shop_knowledges';
    protected $guarded = [];
    protected $appends = [
        'title',
        'keyword',
    ...
    {
        return $this->hasMany(ShopKnowledgesDescription::class, 'shop_knowledges_id', 'id');
    }
    public function getItemsNews($limit = null, $opt = null)
    {
        $query = (new ShopKnowledges)->where('status', 1)->sort();
        if (!(int) $limit) {
            return $query->get();
        } else
            if ($opt == 'paginate') {
    ...
    {
        return route('knowledges.detail', ['alias' => $this->alias]);
    }
